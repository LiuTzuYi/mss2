 ...
 * @Last Modified time: 2019-10-08 11:26:41
 */

/**
 * @memberof carSafety
 * @ngdoc Controller
 * @name vehiclegrpProfileCtrl
...
angular.module('carSafety').controller("vehiclegrpProfileCtrl", ["$scope", "$state", "$stateParams", "$http", "blockUI", "$timeout", "$translate", "user", "Idle", "Keepalive", "httpSuccessCheck", "alertBox", "$window", "$filter", "appConfig", "profileService", "tmhDynamicLocale", "refetchMapService", "$mdDialog", "exportConfig", "chartConfig", "mapConfig", function ($scope, $state, $stateParams, $http, blockUI, $timeout, $translate, user, Idle, Keepalive, httpSuccessCheck, alertBox, $window, $filter, appConfig, profileService, tmhDynamicLocale, refetchMapService, $mdDialog, exportConfig, chartConfig, mapConfig) {
	...
	/*$scope.setTitle = function(){
		window.setTimeout(function(){
			window.document.title = $translate.instant('vehicleGrpProfile');
		}, 50);
	}
	...
	$translate(chartConfig.scoreChartSeriesFullGrp).then(function (translations) {
		angular.forEach(translations, function (value, key) {
			$scope.scoreChartSeriesFullGrp.push(value);
		});
	}, function (translationIds) {
		angular.forEach(translations, function (value, key) {
			$scope.scoreChartSeriesFullGrp.push(value);
	...
	$scope.getWarnSpec = function (warn) {
		...
		$http({
			...
			if (errorphp) {
				alertBox.errorAlert('91507_' + errorphp)
			} else {
	...
	$scope.listDataSelectTexts = {
		buttonDefaultText: $translate.instant('selectGrp'),
	...
	$scope.getWarnVideo = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/profileVideo/vg/' + datebf + '/' + dateaf + '/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.video = false;
				alertBox.errorAlert('91508_01')
			} else {
				var data = response.data.data
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91508_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
	...
	$scope.getCompScore = function (submit) {
		...
		$http({
			...
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91501_01')
			} else {
				$scope.companyCollection = response.data.data
			}
			$scope.getVehGrpScore(submit)
		}, function errorCallback(response) {
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91501_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
			$scope.getVehGrpScore(submit)
	...
	$scope.getVehGrpScore = function (submit) {
		$scope.loadingdata.vehgrp = true;
		var datebf = $filter('date')($scope.dateRange.before, 'yyyy-MM-dd')
		var dateaf = $filter('date')($scope.dateRange.after, 'yyyy-MM-dd')
		$http({
			method: 'GET',
			url: '/api/vehgrpScore/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/g/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.vehgrp = false;
				blockUi.removeClass('block-ui-active block-ui-visible')
				alertBox.errorAlert('91502_01')
				if (submit) {
				...
				if ($scope.vehgrpCollection.length > 0) {
					var tmp = angular.copy($scope.vehgrpCollection[$scope.vehgrpCollection.length - 1])
					...
					if ($scope.companyCollection && $scope.companyCollection.length > 0) {
						...
						$scope.chartSet = profileService.renderChartData(angular.copy($scope.vehgrpCollection), $scope.periodselected, angular.copy($scope.companyCollection))
					} else {
						tmp.total_score_avg = 0
						tmp.grade_avg = 'N/A'
						$scope.chartSet = profileService.renderChartData(angular.copy($scope.vehgrpCollection), $scope.periodselected, new Array())
					}
					...
					if (dateaf !== tmp.acc_date) {
						...
						alertBox.asAtDateAlert('vehicleGrp', $scope.distanceselected, selectedRecordPeriod, tmp.dataAsAt, $scope.periodselected, submit, tmp, func)
					} else {
						func(submit, tmp)
					}
				} else {
					...
					alertBox.noProfileRecordAlert('vehicleGrp', $scope.distanceselected, start, end, $scope.periodselected, submit, $scope.dateRange.after, func)
				}
				$timeout(function(){
					$scope.spiderPoint = $scope.getSpiderPoint();
					$scope.$apply()
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91502_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
			if (submit) {
	...
	$scope.getVehicleScore = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/vehicleScore/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/g/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.veh = false;
				alertBox.errorAlert('91503_01')
			} else {
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91503_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
	...
	$scope.getRank = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/warningRank/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/vg/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.rank = false;
				alertBox.errorAlert('91504_01')
			} else {
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91504_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
	...
	$scope.getBaseInfo = function () {
		$http({
			method: 'GET',
			url: '/api/profileBase/vg/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91505_01')
			} else {
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91505_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
	...
	$scope.getList = function () {
		$http({
			method: 'GET',
			url: '/api/profileList/vg'
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				blockUi.removeClass('block-ui-active block-ui-visible')
				alertBox.errorAlert('91506_01')
			} else {
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91506_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
