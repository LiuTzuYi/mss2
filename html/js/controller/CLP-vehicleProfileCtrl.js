...
angular.module('carSafety').controller("CLP-vehicleProfileCtrl", ["$scope", "$state", "$stateParams", "$http", "blockUI", "$timeout", "$translate", "user", "Idle", "Keepalive", "httpSuccessCheck", "alertBox", "$window", "$filter", "appConfig", "profileService", "$mdDialog", "tmhDynamicLocale", "refetchMapService", "exportConfig", "chartConfig", "mapConfig", function ($scope, $state, $stateParams, $http, blockUI, $timeout, $translate, user, Idle, Keepalive, httpSuccessCheck, alertBox, $window, $filter, appConfig, profileService, $mdDialog, tmhDynamicLocale, refetchMapService, exportConfig, chartConfig, mapConfig) {
	...
	$scope.getVehicleScore = function (submit) {
		...
		$http({
			method: 'GET',
			url: '/api/vehicleScore_CLP/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/i/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
	...
	$scope.getRank = function () {
		...
		$http({
			method: 'GET',
			url: '/api/warningRank_CLP/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/vi/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
