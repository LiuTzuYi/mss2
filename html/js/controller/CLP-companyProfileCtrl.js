...
//	for (var i = 0; i < chartConfig.spiderFieldSet.length; i++){
	...
	// $scope.profileExcelOrderDate = exportConfig.CLP_profileExcelOrderDate.concat($scope.warnFieldArr)
	// $scope.profileExcelOrderName = exportConfig.CLP_profileExcelOrderName.concat($scope.warnFieldArr)
	// $scope.profileExcelOrderDateAct = exportConfig.CLP_profileExcelOrderDateAct.concat($scope.warnFieldArr)
	// $scope.profileExcelOrderNameAct = exportConfig.CLP_profileExcelOrderNameAct.concat($scope.warnFieldArr)
	// $scope.profileExcelOrderDateScore = exportConfig.CLP_profileExcelOrderDateScore.concat($scope.warnFieldArr)
	// $scope.profileExcelOrderNameScore = exportConfig.CLP_profileExcelOrderNameScore.concat($scope.warnFieldArr)
	// $scope.profileExcelOrderRank = exportConfig.CLP_profileExcelOrderNameRank.concat($scope.warnFieldArr)
	// $scope.profileExcelOrderVideoDep = exportConfig.CLP_profileExcelOrderVideoDep
...
//		$scope.chartDataFullscreen = profileService.CLP_renderChartFullscreen(chart, $scope.chartSet, false);
	}
...
//			url: '/api/CLP_profileVideo/c/' + datebf + '/' + dateaf
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
...
//					tmp.spiderset = profileService.CLP_getSpiderData(tmp)
...
//					$scope.recentRecord.spiderset = profileService.CLP_renderEmptySpider()
