 ...
 * @Last Modified time: 2019-10-08 11:22:37
 */

/**
 * @memberof carSafety
 * @ngdoc Controller
 * @name drivergrpProfileCtrl
...
angular.module('carSafety').controller("drivergrpProfileCtrl", ["$scope", "$state", "$stateParams", "$http", "blockUI", "$timeout", "$translate", "user", "Idle", "Keepalive", "httpSuccessCheck", "alertBox", "$window", "$filter", "appConfig", "profileService", "tmhDynamicLocale", "refetchMapService", "$mdDialog", "exportConfig", "chartConfig", "mapConfig", function ($scope, $state, $stateParams, $http, blockUI, $timeout, $translate, user, Idle, Keepalive, httpSuccessCheck, alertBox, $window, $filter, appConfig, profileService, tmhDynamicLocale, refetchMapService, $mdDialog, exportConfig, chartConfig, mapConfig) {
	...
	$scope.getWarnSpec = function (warn) {
		...
		$http({
			...
			if (errorphp) {
				alertBox.errorAlert('91307_' + errorphp)
			} else {
	...
	$scope.getWarnVideo = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/profileVideo/dg/' + datebf + '/' + dateaf + '/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.video = false;
				alertBox.errorAlert('91308_01')
			} else {
				var data = response.data.data
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91308_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
	...
	$scope.getCompScore = function (submit) {
		...
		$http({
			...
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91301_01')
			} else {
				$scope.companyCollection = response.data.data
			}
			$scope.getDrvGrpScore(submit)
		}, function errorCallback(response) {
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91301_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
			$scope.getDrvGrpScore(submit)
	...
	$scope.getDrvGrpScore = function (submit) {
		$scope.loadingdata.drvgrp = true;
		var datebf = $filter('date')($scope.dateRange.before, 'yyyy-MM-dd')
		var dateaf = $filter('date')($scope.dateRange.after, 'yyyy-MM-dd')
		$http({
			method: 'GET',
			url: '/api/drvgrpScore/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/g/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.drvgrp = false;
				blockUi.removeClass('block-ui-active block-ui-visible')
				alertBox.errorAlert('91302_01')
				if (submit) {
				...
				if ($scope.drvgrpCollection.length > 0) {
					var tmp = angular.copy($scope.drvgrpCollection[$scope.drvgrpCollection.length - 1])
					//tmp.label = profileService.getscoreLabel(tmp.total_score)
					...
					if ($scope.companyCollection && $scope.companyCollection.length > 0) {
						...
						$scope.chartSet = profileService.renderChartData(angular.copy($scope.drvgrpCollection), $scope.periodselected, angular.copy($scope.companyCollection))
					} else {
						tmp.total_score_avg = 0
						tmp.grade_avg = 'N/A'
						$scope.chartSet = profileService.renderChartData(angular.copy($scope.drvgrpCollection), $scope.periodselected, new Array())
					}
					...
					if (dateaf !== tmp.acc_date) {
						...
						alertBox.asAtDateAlert('driverGrp', $scope.distanceselected, selectedRecordPeriod, tmp.dataAsAt, $scope.periodselected, submit, tmp, func)
					} else {
						func(submit, tmp)
					}
				} else {
					...
					alertBox.noProfileRecordAlert('driverGrp', $scope.distanceselected, start, end, $scope.periodselected, submit, $scope.dateRange.after, func)
				}
				$timeout(function(){
					$scope.spiderPoint = $scope.getSpiderPoint();
					$scope.$apply()
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91302_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
			if (submit) {
	...
	$scope.getDriverScore = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/driverScore/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/g/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.drv = false;
				alertBox.errorAlert('91303_01')
			} else {
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91303_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
	...
	$scope.getRank = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/warningRank/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/dg/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.rank = false;
				alertBox.errorAlert('91304_01')
			} else {
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91304_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
	...
	$scope.getBaseInfo = function () {
		$http({
			method: 'GET',
			url: '/api/profileBase/dg/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91305_01')
			} else {
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91305_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
	...
	$scope.getList = function () {
		$http({
			method: 'GET',
			url: '/api/profileList/dg'
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				blockUi.removeClass('block-ui-active block-ui-visible')
				alertBox.errorAlert('91306_01')
			} else {
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91306_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
