 ...
 * @Last Modified time: 2019-10-08 11:26:59
 */

/**
 * @memberof carSafety
 * @ngdoc Controller
 * @name vehicleProfileCtrl
...
angular.module('carSafety').controller("vehicleProfileCtrl", ["$scope", "$state", "$stateParams", "$http", "blockUI", "$timeout", "$translate", "user", "Idle", "Keepalive", "httpSuccessCheck", "alertBox", "$window", "$filter", "appConfig", "profileService", "$mdDialog", "tmhDynamicLocale", "refetchMapService", "exportConfig", "chartConfig", "mapConfig", function ($scope, $state, $stateParams, $http, blockUI, $timeout, $translate, user, Idle, Keepalive, httpSuccessCheck, alertBox, $window, $filter, appConfig, profileService, $mdDialog, tmhDynamicLocale, refetchMapService, exportConfig, chartConfig, mapConfig) {
	...
	/*$scope.setTitle = function(){
		window.setTimeout(function(){
			window.document.title = $translate.instant('vehicleProfile');
		}, 50);
	}
	...
	$translate(chartConfig.scoreChartSeriesFullVeh).then(function (translations) {
		angular.forEach(translations, function (value, key) {
			$scope.scoreChartSeriesFullVeh.push(value);
		});
	}, function (translationIds) {
		angular.forEach(translations, function (value, key) {
			$scope.scoreChartSeriesFullVeh.push(value);
	...
	$scope.getWarnSpec = function (warn) {
		...
		$http({
			...
			if (errorphp) {
				alertBox.errorAlert('91607_' + errorphp)
			} else {
	...
	$scope.getWarnVideo = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/profileVideo/vi/' + datebf + '/' + dateaf + '/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.video = false;
				alertBox.errorAlert('91601_01')
			} else {
				var data = response.data.data
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91601_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
	...
	$scope.getCompScore = function (submit) {
		...
		$http({
			...
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91602_01')
			} else {
				$scope.companyCollection = response.data.data
			}
			$scope.getVehicleScore(submit)
		}, function errorCallback(response) {
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91602_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
			$scope.getVehicleScore(submit)
	...
	$scope.getVehicleScore = function (submit) {
		$scope.loadingdata.veh = true;
		var datebf = $filter('date')($scope.dateRange.before, 'yyyy-MM-dd')
		var dateaf = $filter('date')($scope.dateRange.after, 'yyyy-MM-dd')
		$http({
			method: 'GET',
			url: '/api/vehicleScore/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/i/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.veh = false;
				blockUi.removeClass('block-ui-active block-ui-visible')
				alertBox.errorAlert('91603_01')
				if (submit) {
				...
				if ($scope.vehCollection.length > 0) {
					var tmp = angular.copy($scope.vehCollection[$scope.vehCollection.length - 1])
					...
					if ($scope.companyCollection && $scope.companyCollection.length > 0) {
						...
						$scope.chartSet = profileService.renderChartData(angular.copy($scope.vehCollection), $scope.periodselected, angular.copy($scope.companyCollection))
					} else {
						tmp.total_score_avg = 0
						tmp.grade_avg = 'N/A'
						$scope.chartSet = profileService.renderChartData(angular.copy($scope.vehCollection), $scope.periodselected, new Array())
					}
					...
					if (dateaf !== tmp.acc_date) {
						...
						alertBox.asAtDateAlert('vehicle', $scope.distanceselected, selectedRecordPeriod, tmp.dataAsAt, $scope.periodselected, submit, tmp, func)
					} else {
						func(submit, tmp)
					}
				} else {
					...
					alertBox.noProfileRecordAlert('vehicle', $scope.distanceselected, start, end, $scope.periodselected, submit, $scope.dateRange.after, func)
				}
				$timeout(function(){
					$scope.spiderPoint = $scope.getSpiderPoint();
					$scope.$apply()
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91603_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
			if (submit) {
	...
	$scope.getRank = function () {
		...
		$http({
			method: 'GET',
			url: '/api/warningRank/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/vi/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				$scope.loadingdata.rank = false;
				alertBox.errorAlert('91604_01')
			} else {
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91604_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
	...
	$scope.getBaseInfo = function () {
		$http({
			method: 'GET',
			url: '/api/profileBase/vi/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91605_01')
			} else {
			...
			var errorphp = httpSuccessCheck.checkerrorHttp(response)
			if (errorphp) {
				alertBox.errorAlert('91605_' + errorphp)
			} else {
				alertBox.errorAlert(response.data.code)
			}
		});
			...
			if (errorphp) {
				...
				if ($state.params.preselected && $state.params.preview && $state.params.predate) {
					...
					if ($scope.labelselected) {
						...
						blockUi.removeClass('block-ui-active block-ui-visible')
						alertBox.profileIndVisibilityAlert('vehicle')
					}
