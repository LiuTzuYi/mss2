...
angular.module('carSafety').controller("CLP-drivergrpProfileCtrl", ["$scope", "$state", "$stateParams", "$http", "blockUI", "$timeout", "$translate", "user", "Idle", "Keepalive", "httpSuccessCheck", "alertBox", "$window", "$filter", "appConfig", "profileService", "tmhDynamicLocale", "refetchMapService", "$mdDialog", "exportConfig", "chartConfig", "mapConfig", function ($scope, $state, $stateParams, $http, blockUI, $timeout, $translate, user, Idle, Keepalive, httpSuccessCheck, alertBox, $window, $filter, appConfig, profileService, tmhDynamicLocale, refetchMapService, $mdDialog, exportConfig, chartConfig, mapConfig) {
	...
	$scope.getDrvGrpScore = function (submit) {
		...
		$http({
			method: 'GET',
			url: '/api/drvgrpScore_CLP/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/g/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
	...
	$scope.getDriverScore = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/driverScore_CLP/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/g/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
	...
	$scope.getRank = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/warningRank_CLP/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/dg/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
