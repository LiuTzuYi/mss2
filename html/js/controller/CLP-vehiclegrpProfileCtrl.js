 ...
 * @memberof carSafety
 * @ngdoc Controller
 * @name CLP-vehiclegrpProfileCtrl
...
angular.module('carSafety').controller("CLP-vehiclegrpProfileCtrl", ["$scope", "$state", "$stateParams", "$http", "blockUI", "$timeout", "$translate", "user", "Idle", "Keepalive", "httpSuccessCheck", "alertBox", "$window", "$filter", "appConfig", "profileService", "tmhDynamicLocale", "refetchMapService", "$mdDialog", "exportConfig", "chartConfig", "mapConfig", function ($scope, $state, $stateParams, $http, blockUI, $timeout, $translate, user, Idle, Keepalive, httpSuccessCheck, alertBox, $window, $filter, appConfig, profileService, tmhDynamicLocale, refetchMapService, $mdDialog, exportConfig, chartConfig, mapConfig) {
	...
	$scope.getVehGrpScore = function (submit) {
		...
		$http({
			method: 'GET',
			url: '/api/vehgrpScore_CLP/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/g/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
	...
	$scope.getVehicleScore = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/vehicleScore_CLP/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/g/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
	...
	$scope.getRank = function (date) {
		...
		$http({
			method: 'GET',
			url: '/api/warningRank_CLP/' + $scope.periodselected.toUpperCase() + '/' + ($scope.distanceselected*10*100000)/10 + '/' + datebf + '/' + dateaf + '/vg/' + $scope.labelselected.id
		}).then(function successCallback(response) {
			var errorphp = httpSuccessCheck.checkHttp(response)
			if (errorphp) {
