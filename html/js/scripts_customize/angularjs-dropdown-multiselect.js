...
* @Last Modified time: 2019-05-31 16:11:15
*/
	...
	function dropdownMultiselectDirective() {
		return {
			...
			templateUrl: function(element, attrs) {
		      return attrs.templateUrl || 'app/component/angularjs-dropdown-multiselect.html';
		    }
		};
	}
...
/***/ function(module, exports) {
	...
	dropdownMultiselectController.$inject = ["$scope", "$element", "$filter", "$document", "$timeout"];
	Object.defineProperty(exports, "__esModule", {
		value: true
	});
	...
	*/
	...
	function dropdownMultiselectController($scope, $element, $filter, $document, $timeout) {
		'ngInject';
		...
		function toggleDropdown() {
			if ($scope.open) {
				...
				input.searchFilter = $scope.searchFilter || ''
				$scope.open = true;
				if(custom){
					$timeout(function() {
		...
		function deselectAll() {
			...
			for (var i = searchResult.length - 1; i >= 0; i--) {
				var exists = $scope.selectedModel.indexOf(searchResult[i]);
				if (exists !== -1) {
					$scope.selectedModel.splice(exists, 1);
				}
			}
			//$scope.selectedModel.splice(0, $scope.selectedModel.length);
			if (!dontSendEvent) {
